...
//תמלא את המספרים. בהתחלה תחפש את הריק, אחרי שתמצא ריק - תנסה להוסיף לשם מספר נכון מ1 עד 9 עם בדיקה שלא קיים מספר כזה בשורה, וככה תמשיך עד שכל המקומות יהיו עם מספר. במקרה ויהיה שגיעה - תחזור צעד אחד אחורה ותחפש שוב את הסיפרה הנכונה.
function resolveSudoku(grid) {
    ...
    for (let i = 0; i < numbers.length; i++) {
        ...
        if (resolveSudoku(grid)) return true; //אם מילוי נכון

        grid[emptyCell.row][emptyCell.column] = null; //אם מילוי שגוי
    }
}
...
// תחזיר טרו אך ורק אם כל שלושתם תחזירו טרו
