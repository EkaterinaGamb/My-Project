...
function onCellClick(clickedCell, index) {
    cells.forEach(cell => cell.classList.remove('selected', 'highlighted', 'error')); //כדי להוריד את הצבע של בחירה
    ...
    cells.forEach(cell => {
        if (cell.innerHTML === clickedCell.innerHTML) cell.classList.add('selected'); //מסמן את כל מקומות עם הספרה הזאת
    });
}
...
function initRemover() { //מחיקה מהתא
